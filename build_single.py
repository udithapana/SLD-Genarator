...
h = re.sub(r'<script src="([^"]+)"></script>', lambda m: m.group(0) if m.group(1).startswith('http') else '<script>' + open(m.group(1), encoding='utf-8').read().replace('</script>', '<\\/script>') + '</script>', h)
